...
dummy <- dummyVars( ~ Occupation + BMI.Category , data = train)
...
# Test Set
dummy1 <- dummyVars( ~ Occupation + BMI.Category , data = test)
...
vs.train <- vs.train[, -c(col_index1, col_index2)]
vs.test <- vs.test[, -c(col_index1, col_index2)]
...
glmnFit <- train(x = vs.train[, 1:8, 10:19], 
...
testResults <- data.frame(obs = vs.test$Sleep.Disorder.Binary, 
                          PGLM = predict(glmnFit, vs.test[,1:8, 10:19]))

```
#### Penalized GLM resulted in an accuracy of 90.99% sensitivity of 0.8913, and specificity of 0.9231
...
nnetFit <- train(x = vs.train[, 1:8, 10:19], 
                 y = vs.train$Sleep.Disorder.Binary,
                method = "nnet",
                tuneGrid = nnetGrid,
                metric = "ROC",
...
```{r}
testResults$NNET <- predict(nnetFit, vs.test[,1:8, 10:19])

```

#### Neural Network Model Accuracy is 92.79%, with a sensitivty of 0.9130, and specificity of 0.9385. 
...
gbmFit <- train(x = vs.train[, 1:8, 10:19], 
                 y = vs.train$Sleep.Disorder.Binary,
                method = "gbm",
                tuneGrid = gbmGrid,
                verbose = FALSE,
...
```{r}
testResults$gbm <- predict(gbmFit, vs.test[, 1:8, 10:19])
```
#### Boosting Model had an accuracy of 90.99%, with  sensitivity of 0.9130, and specificity of 0.8923
...
#### Random Forest - 92.79% accuracy, with a sensitivity of 0.9130, and specificity of 0.9385
...
# Random Forest
rf_model <- train(Sleep.Disorder.Binary ~ ., data = trainJD, method = "rf", 
    trControl = trainControl(method = "cv", number = 5), ntree = 100)
rf_model$finalModel

rf_pred <- predict(rf_model, newdata = testJD[,-11])
rf_cm <- confusionMatrix(rf_pred, testJD$Sleep.Disorder.Binary)
rf_cm
```

```{r}
testResults$RF <- predict(rf_model, testJD[,-11])
```


#### Bagged ADA Boost - 92.79%, sensitivity of 0.9130, and specificity of 0.9385.
...
bag_pred <- predict(bag_model, newdata = testJD[,-11])
bag_predict <- bag_pred$class
bag_predict <- as.factor(bag_predict)
dim(bag_predict)

#Calculate model statistics for Bagged AdaBoost
bag_cm <- confusionMatrix(bag_predict, testJD$Sleep.Disorder.Binary)
bag_cm
```

```{r}
testResults$boost <- bag_predict
```


#### XGBoost - Accuracy 92.79%, sensitivity 0.9130, specificity 0.9385
...
dtrain <- xgb.DMatrix(data = as.matrix(trainJD[, -11]), label = as.matrix(trainJD[, 11]))
...
dtest <- xgb.DMatrix(data = as.matrix(testJD[, -11]), label = as.matrix(testJD[, 11]))
...
lrFit <- train(x = trainDA[,1:8, 10:19], 
...
#### Logistic Regression Model Accuracy = 91.89%, sensitivity 0.9130 specificity 0.9231
...
testResults$lrFit <- predict(lrFit, testDA[,1:8, 10:19])
...
set.seed(13)
ldaFit <- train(trainDA[,1:8, 10:19], 
               y = trainDA$Sleep.Disorder.Binary,
                method = "lda",
                preProc = c("center", "scale"),
                metric = "ROC",
...
#### Linear Discriminant Analysis Model Accuracy = 921.89% sensitivity 0.9130 specificity 0.9130

```{r}

suppressWarnings({

ldaPred <- predict(ldaFit, newdata = testDA[,1:8, 10:19])
...
testResults$LDA <- predict(ldaFit, testDA[,1:8, 10:19])
...
set.seed(13)
glmnFitDA <- train(trainDA[,1:8, 10:19], 
               y = trainDA$Sleep.Disorder.Binary,
                 method = "glmnet",
                 tuneGrid = glmnGridDA,
                 preProc = c("center", "scale"),
...
#### Nearest Shrunken Centroids Model Accuracy: 90.99% sensitivity 0.8913 specificity 0.9231

```{r}

nscPred <- predict(glmnFit, newdata = testDA[,1:8, 10:19])
...
testResults$glmnetDA <- predict(glmnFitDA, testDA[,1:8, 10:19])
testResults
...
legend("bottomright", legend = c("Boosting", "PGLM", "NeuralNet", "LogReg"),
...
confusionMatrix(testResults$boost, testResults$obs, positive = "Yes")
